...
(function (dataArchive) {
    ...
    async function handleLoad(_event) {
        ...
        Object.entries(parsedData.data).forEach((entry) => {
            ...
            trashbin.addEventListener("click", async function () {
                ...
                this.parentNode.parentNode.removeChild(this.parentNode);
                let response = await fetch("https://webuser.hs-furtwangen.de/~stolzman/Database/" +
                    "?" +
    ...
    async function sendTask(_event) {
        let task = getData();
        let params = "?command=insert&collection=TaskData&data=" + JSON.stringify(task);
        //
        _event.preventDefault();
        let response = await fetch("https://webuser.hs-furtwangen.de/~stolzman/Database/" + params); //Send data to Databank
        if (response.ok) {
            console.log(response);
    ...
    async function getTasks() {
        ...
        let request = await fetch("https://webuser.hs-furtwangen.de/~stolzman/Database/?" + params);
    ...
    document
        ...
        let task = getData();
        e.preventDefault();
        let params = "?command=update&collection=TaskData&id=" +
            activeID +
            "&data=" +
            JSON.stringify(task);
        let response = await fetch("https://webuser.hs-furtwangen.de/~stolzman/Database/" + params);
        if (response.ok) {
            location.reload();
        }
    });
